...
Html5Player.prototype.auth = function auth() {
  ...
  this.streamingPlayer.callbackHandler('trackEnded', function() {
    window.parent.postMessage(
      { type: 'playevent', data: { id: that.currentTrack, code: 'PlayComplete', playing: false } },
      '*'
    )
...
Html5Player.prototype.play = function play(o, t) {
  ...
  this.currentTrack = o
  window.parent.postMessage(
    { type: 'playevent', data: { id: o, code: 'PlayStarted', playing: true } },
    '*'
  )
}
Html5Player.prototype.pause = function pause() {
  this.streamingPlayer.pause()
  window.parent.postMessage(
    { type: 'playevent', data: { id: this.currentTrack, code: 'Paused', playing: false } },
    '*'
  )
}
...
Html5Player.prototype.resume = function resume() {
  ...
  window.parent.postMessage(
    { type: 'playevent', data: { id: this.currentTrack, code: 'PlayStarted', playing: true } },
    '*'
  )
}
...
Html5Player.prototype.previous = function previous() {
  ...
  if (this.played.length === 1) {
    ...
    window.parent.postMessage(
      { type: 'playevent', data: { id: this.played[0], code: 'PlayStarted', playing: true } },
      '*'
    )
  } else {
