      ...
      if (id && typeof id === 'string') {
        ...
        if (shouldLoadHTML5Engine()) {
          ...
          if (!checkRequirements()) {
            throw new Error(
              'Cookies or localStorage is not enabled. Napster.js will not work properly without it.'
            )
          }
          ...
          if (d.length === 0) {
            $(function() {
              var f = $('<iframe></iframe>')
                ...
                .attr(
                  'src',
                  'http://api.napster.com/v1.1/player/index.html?apikey=' + options.consumerKey
                )
                .attr('frameborder', 'no')
    ...
    return {
      post: function(method, args) {
        ...
        win.contentWindow.postMessage(
          { method: method, args: Napster.util.jsonClean(args || {}) },
          '*'
        )
      },
    }
